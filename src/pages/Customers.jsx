...
export const Customers = () => {
  ...
  const handleCreateCustomer = async () => {
    ...
    const newCustomerData = {
      // id will be generated by backend if omitted, or we can send Date.now() if we want specific
      name: step1Data.customerName,
      email: step1Data.emailId,
      customerId: step1Data.customerId, // Mapped to externalId in backend
      billingAddress: step1Data.billingAddress,
      currency: step2Data.currency,
      accountId: step2Data.accountId, // Stored in metadata
      aliases: step2Data.aliases, // Stored in metadata
      accountName: step2Data.accountName
    };

    try {
      // Call backend
      const savedCustomer = await api.createCustomer(newCustomerData);

      if (savedCustomer.error) {
        throw new Error(savedCustomer.error);
      }
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {state.customers
                    .filter(customer => !customer.metadata?.parentCustomerId) // Only show parent customers, not child accounts
                    .map((customer) => (
                      <tr
                        key={customer.id}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{customer.externalId || customer.customerId}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div>
                          <div className="text-sm font-medium text-gray-900">{customer.name}</div>
                          <div className="text-xs text-gray-500">{customer.externalId || customer.customerId}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {(() => {
                        const customerSubscriptions = state.subscriptions?.filter(
                        ...
                        const productNames = customerSubscriptions.map(sub => {
                          const plan = state.plans?.find(p => p.id === sub.planId);
                          const product = state.products?.find(p => p.id === plan?.productId);
                          return product?.name || plan?.name || 'Unknown';
                        });

                        return (
                          <div className="flex flex-col gap-1">
                            {productNames.map((name, idx) => (
                              <span key={idx} className="px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded text-xs inline-block w-fit">
                                {name}
                              </span>
                            ))}
                          </div>
